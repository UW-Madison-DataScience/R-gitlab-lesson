...
In GitLab you can check your username by clicking on your profile icon in the upper left panel. It will say `@FIRSTNAME.LASTNAME` or something to that effect.
...
In GitLab, we need to click the blue "Code" button on the upper right of our GitLab project.
...
To stage the files in Rstudio, we can click the button in the "Staged" column of the Git pane.
When we do this, the status of each file changes from "? ?" to "A" for "Added to the stage".
Then to commit them, we can click the commit button in the menu of the Git pane.
...
in the right hand menu or by clicking "Code" and "Commits" buttons on the left hand menus.
